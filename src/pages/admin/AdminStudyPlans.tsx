...
const AdminStudyPlans = () => {
  const { isAdmin, loading } = useAuth();
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [pendingDelete, setPendingDelete] = useState<Row | null>(null);
  const [form, setForm] = useState({ title: "", program: "", description: "", file_url: "" });
  ...
  const reset = () => {
    setForm({ title: "", program: "", description: "", file_url: "" });
    setEditingId(null);
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        const { error } = await supabase.from("study_plans").update(payload).eq("id", editingId);
        if (error) throw error;
      } else {
        ...
        preview={
          pendingDelete ? (
            <div className="rounded-md border border-border bg-muted/40 p-3 text-sm">
              ...
              <p className="text-muted-foreground">{pendingDelete.program ?? "—"}</p>
            </div>
          ) : null
        }
      />
